...
# === Config ===
VERSION=4
SUBSET_DIR = f"datasets/coco_subset/version_{VERSION}"
...
def main():
    coco = COCO(COCO_ANNOTATION_FILE) # COCO API 로딩


    for split in SPLITS:
        # Load split metadata
        with open(os.path.join(SUBSET_DIR, "meta",f"subset_meta_{split}.json")) as f:
            meta = json.load(f)
